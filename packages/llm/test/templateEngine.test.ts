...
import { AVAILABLE_PROMPT_VARIABLES, renderPromptTemplate } from '../src/templateEngine.js';
...
指示: {instruction}`;
  ...
  it('instruction と customInstruction のエイリアスが相互に機能すること', () => {
    ...
    const res1 = renderPromptTemplate(template, { customInstruction: 'ハードボイルドに' });
    expect(res1).toBe('指示: ハードボイルドに / カスタム: ハードボイルドに');

    const res2 = renderPromptTemplate(template, { instruction: 'ファンタジー風に' });
