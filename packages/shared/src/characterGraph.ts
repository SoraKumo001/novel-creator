 ...
 */
export function generateCharacterMermaidGraph(characters: CharacterGraphNode[]): string {
  ...
  for (const char of characters) {
    ...
    if (typeof rels === 'string' && rels.trim()) {
      ...
      for (const [targetKey, label] of Object.entries(rels as Record<string, unknown>)) {
        const targetId = nameToId.get(targetKey.trim()) || idToSanitized.get(targetKey.trim());
