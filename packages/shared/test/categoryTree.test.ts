...
describe('categoryTree utility', () => {
  describe('parseCategoryPath', () => {
    it('スラッシュ区切りのパスを配列に分解すること', () => {
      ...
      expect(parseCategoryPath('世界観／魔法／火属性')).toEqual(['世界観', '魔法', '火属性']);
    });
  ...
  describe('formatCategoryPath', () => {
    it('セグメント配列をスラッシュ区切り文字列に整形すること', () => {
      expect(formatCategoryPath(['採取ギルド', '採取メンバー'])).toBe('採取ギルド / 採取メンバー');
  ...
  describe('buildCategoryTree', () => {
    const characters = [
      { id: '1', name: 'アイン', category: '採取ギルド / 採取メンバー' },
      { id: '2', name: 'ロイク', category: '採取ギルド / 採取メンバー' },
      { id: '3', name: 'ユーグ', category: '採取ギルド / 幹部 / ギルド長' },
      { id: '4', name: 'セドリック', category: '採取ギルド / 幹部 / 副ギルド長' },
      { id: '5', name: 'アリオス', category: '騎士団 / 団長' },
      { id: '6', name: 'ウォレン', category: '未分類' },
    ];
    ...
    it('階層構造のツリーノードを正しく構築すること', () => {
      ...
      expect(tree.map((n) => n.name)).toEqual(['騎士団', '採取ギルド', '未分類']);

      const guildNode = tree.find((n) => n.name === '採取ギルド')!;
      expect(guildNode).toBeDefined();
      expect(guildNode.totalCount).toBe(4);
      expect(guildNode.children.map((c) => c.name)).toEqual(['幹部', '採取メンバー']);

      const memberNode = guildNode.children.find((c) => c.name === '採取メンバー')!;
      ...
      expect(executiveNode.children.map((c) => c.name)).toEqual(['ギルド長', '副ギルド長']);
