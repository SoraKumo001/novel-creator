...
`;
  ...
  it("findStoryOutlineSectionByLine でカーソル行のセクションを特定できること", () => {
    ...
    const introLine = lines.findIndex((l) =>
      l.includes("少年が魔法適性ゼロと判定され")
    );
  ...
  it("STORY_OUTLINE_TEMPLATES に起承転結・三幕構成などのテンプレートが存在すること", () => {
    ...
    expect(
      STORY_OUTLINE_TEMPLATES.some((t) => t.id === "standard_kishotenketsu")
    ).toBe(true);
    expect(
      STORY_OUTLINE_TEMPLATES.some((t) => t.id === "three_act_structure")
    ).toBe(true);
  });
