...
export const foreshadowings = pgTable(
  "foreshadowings",
  {
    ...
    resolvedSectionId: uuid("resolved_section_id").references(
      () => sections.id,
      {
        onDelete: "set null",
      }
  ...
  (t) => [
    index("foreshadowings_novel_id_idx").on(t.novelId),
    index("foreshadowings_placed_section_id_idx").on(t.placedSectionId),
    index("foreshadowings_resolved_section_id_idx").on(t.resolvedSectionId),
  ]
);
