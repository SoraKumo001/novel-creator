...
import { QUICK_PROMPTS } from "@/context/chatUiTypes.js";
...
export function ChatDrawer() {
  const { drawer, history, input: inputState } = useChatDrawer();

  if (!drawer.isOpen) {
    return null;
  }

  const isFull = drawer.drawerWidth === "full";

  // 幅クラス（標準 / ワイド）
  const widthClasses =
    drawer.drawerWidth === "wide"
  ...
  return (
    <aside
      ...
      role={drawer.isDialog ? "dialog" : undefined}
      aria-modal={drawer.isDialog ? "true" : undefined}
    >
      {/* ヘッダー */}
      <header className="flex shrink-0 items-center justify-between border-border border-b bg-surface-raised/90 px-4 py-3 backdrop-blur">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex min-w-0 items-center gap-1.5 text-[11px] text-muted-foreground">
              ...
              <span
                ...
                title={
                  drawer.currentNovelTitle
                    ? `対象の小説: ${drawer.currentNovelTitle}`
                    : "対象: 全般相談（小説を選んでいません）"
                }
              >
                📚 {drawer.currentNovelTitle ?? "全般相談"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-1 max-sm:gap-0.5">
          ...
          <button
            type="button"
            onClick={() =>
              drawer.handleLayoutModeChange(
                drawer.layoutMode === "docked" ? "overlay" : "docked"
              )
            }
          ...
          <button
            ...
            onClick={() => {
              const next =
                drawer.drawerWidth === "normal"
                  ? "wide"
                  : drawer.drawerWidth === "wide"
                    ? "full"
                    : "normal";
              drawer.handleWidthChange(next);
            }}
          ...
          <button
            type="button"
            onClick={history.handleStartNewChat}
          ...
          <button
            type="button"
            onClick={() => history.setShowHistoryView((prev) => !prev)}
            aria-pressed={history.showHistoryView}
            aria-label={
              history.showHistoryView ? "相談内容に戻る" : "相談履歴一覧を開く"
            }
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-border border-b bg-surface-raised/50 px-4 py-2 text-xs">
        <div className="flex min-w-0 items-center gap-1.5">
          ...
          <Select
            ...
            value={history.selectedNovelId ?? ""}
            onChange={(e) =>
              history.handleNovelChange(e.target.value ? e.target.value : null)
            }
            title={
              drawer.isStreaming
                ? "返信中です。切り替えると返信が止まります（確認します）"
                : inputState.input.trim()
        ...
        >
          <LLMModelSelector
            value={history.selectedModelConfigId}
            onChange={history.handleModelChange}
            size="sm"
          />
        </div>
      </div>
      ...
      {history.showHistoryView ? (
        <ChatSessionList
          ...
          currentNovelTitle={drawer.currentNovelTitle}
          pinnedIds={history.pinnedIds}
          onTogglePin={history.togglePin}
          onSelectSession={history.handleSelectSession}
          onSaveTitle={history.onSaveTitle}
          onDeleteSession={history.onDeleteSession}
          onStartNewChat={history.handleStartNewChat}
        />
      ) : (
        /* メッセージチャットビュー */
        <>
          <div className="relative flex min-h-0 flex-1 flex-col">
            <div
              ref={drawer.messagesContainerRef}
              onScroll={drawer.handleMessagesScroll}
              role="log"
              aria-live="polite"
              aria-busy={drawer.isStreaming}
              aria-label="チャットメッセージ一覧"
              className="flex-1 space-y-4 overflow-y-auto p-4"
            >
              {drawer.messages.length === 0 && !drawer.streamingContent && (
                <ChatWelcomePanel
                  prompts={QUICK_PROMPTS}
                  isStreaming={drawer.isStreaming}
                  onQuickPrompt={(qp) => void inputState.handleQuickPrompt(qp)}
                />
              )}
              ...
              {drawer.messages.map((m) => (
                <ChatMessageItem
                  ...
                  message={m}
                  isFull={isFull}
                  copiedId={drawer.copiedId}
                  onCopy={(content, id) => void drawer.handleCopy(content, id)}
                />
              ))}
              ...
              {drawer.isStreaming && (
                <StreamingStatus
                  streamingContent={drawer.streamingContent}
                  streamingParts={drawer.streamingParts}
              ...
              {drawer.error && (
                <ChatErrorPanel
                  error={drawer.error}
                  lastPrompt={drawer.lastPrompt}
                  isStreaming={drawer.isStreaming}
                  failedDraft={inputState.failedDraft}
                  onClose={drawer.clearError}
                  onRestoreDraft={inputState.restoreFailedDraft}
                  onRetry={() => {
                    drawer.clearError();
                    void drawer.retryLastMessage();
                  }}
                />
              )}
            </div>

            {drawer.showJumpButton && !history.showHistoryView && (
              <button
                type="button"
                onClick={drawer.scrollToBottom}
          ...
          {/* 入力フォーム */}
          <ChatInputBar
            chatFocus={inputState.chatFocus}
            input={inputState.input}
            isStreaming={drawer.isStreaming}
            textareaRef={inputState.textareaRef}
            failedDraft={inputState.failedDraft}
            onConsumeFocus={inputState.consumeFocus}
            onTextareaInput={inputState.handleTextareaInput}
            onKeyDown={inputState.handleKeyDown}
            onSend={() => void inputState.handleSend()}
            onAbort={() => void drawer.abortStream()}
            onRestoreDraft={inputState.restoreFailedDraft}
            onDismissFailedDraft={inputState.dismissFailedDraft}
          />
        </>
      )}
    </aside>
