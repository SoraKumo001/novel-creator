...
export function getProviderBadge(provider: string) {
  switch (provider) {
    ...
    case 'google':
      return { icon: '🔵', label: 'Google', bg: 'bg-blue-500/10 text-blue-600 dark:text-blue-400' };
    case 'ollama':
      return {
        icon: '🦙',
        label: 'Ollama',
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      <select
        ...
        className={`rounded-md border border-border bg-surface text-foreground font-medium transition focus:border-primary focus:outline-hidden focus:ring-1 focus:ring-primary ${
          isSmall ? 'py-1 px-2 text-xs' : 'py-1.5 px-3 text-sm'
        }`}
      >
        {allowDefault && (
          <option value="">
            {defaultConfig ? `⚙️ デフォルト (${defaultConfig.name})` : '⚙️ デフォルトモデル'}
          </option>
        )}
        {configs.map((cfg) => {
          ...
          return (
            <option key={cfg.id} value={cfg.id}>
              {badge.icon} {cfg.name} ({cfg.modelId}) {cfg.isDefault ? '★デフォルト' : ''}
            </option>
          );
        })}
      </select>
