...
export function AIProgressIndicator({
  ...
  if (variant === "inline") {
    return (
      <div
        className={`flex items-center justify-between gap-3 rounded-lg border border-primary/30 bg-primary/5 px-3 py-2 text-primary text-xs ${className}`}
      >
        <div className="flex min-w-0 flex-1 items-center gap-2">
          ...
          {isDeterminate && (
            <span className="shrink-0 font-semibold text-[11px] text-primary">
              {calculatedPercent}%
            </span>
          )}
        </div>
        {onCancel && (
          <Button
            ...
            variant="secondary"
            onClick={onCancel}
            className="h-7 shrink-0 py-1 text-xs"
          >
            {cancelLabel}
          </Button>
        )}
  ...
  if (variant === "compact") {
    return (
      <div
        className={`flex items-center gap-2 text-primary text-xs ${className}`}
      >
...
function IndeterminateBar() {
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      <style>
        {
          "@keyframes ai-indeterminate { 0% { left: -35%; } 100% { left: 100%; } }"
        }
      </style>
    </div>
  );
