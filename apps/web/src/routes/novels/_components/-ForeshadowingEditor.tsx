...
export function ForeshadowingEditor({ novelId, foreshadowingId }: ForeshadowingEditorProps) {
  ...
  useEffect(() => {
    ...
    if (populatedIdRef.current === targetForeshadowing.id) return;
  ...
  async function handleGenerate() {
    ...
    try {
      const currentDraft =
        category || title || description ? { category, title, description, status } : undefined;
  ...
  return (
    <EntityEditorShell
      ...
      generateLabel={
        isEdit || category || title || description ? 'AIで伏線を更新' : 'AIでドラフト生成'
      }
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 flex-col space-y-1.5">
        <div className="flex items-center justify-between">
          <label className="text-xs font-semibold text-foreground">詳細メモ（Markdown 対応）</label>
