...
import { AIProgressIndicator, formatElapsed } from '../src/components/AIProgressIndicator.js';
...
describe('AIProgressIndicator', () => {
  it('パネルバリアントでステージと説明、不定プログレスバーを描画する', () => {
    ...
    expect(screen.getByText('設定と人物を考慮して構成案を作成しています')).toBeInTheDocument();
  ...
  it('決定型プログレスバーでパーセンテージと件数を表示する', () => {
    render(
      <AIProgressIndicator stage="章構成を解析中..." current={3} total={10} variant="panel" />,
    );
  ...
  it('キャンセルボタンが押されたとき onCancel コールバックを呼び出す', () => {
    ...
    render(<AIProgressIndicator stage="処理中" onCancel={handleCancel} cancelLabel="中止する" />);
