...
describe('extractToolInvocations (AI SDK v7 パーツ形式)', () => {
  ...
  it('text パーツや step-start などツール以外のパーツは無視する', () => {
    const parts = [
      ...
      { type: 'tool-getNovelInfo', toolCallId: 'c1', state: 'input-available', input: {} },
    ];
  ...
  it('toolCallId が無い / 型が不正なパーツはスキップする', () => {
    const parts = [
      ...
      { type: 'tool-getSettings', toolCallId: 'ok-1', state: 'input-available', input: {} },
    ];
    const extracted = extractToolInvocations(parts);
    expect(extracted).toHaveLength(1);
  ...
  it('state 分岐: input-streaming では input が未定義でも抽出できる', () => {
    const parts = [{ type: 'tool-getPlotAndChapters', toolCallId: 'c1', state: 'input-streaming' }];
    const [item] = extractToolInvocations(parts);
  ...
  it('複数ツールパーツを出現順に抽出する', () => {
    ...
    const extracted = extractToolInvocations(parts);
    expect(extracted.map((i) => i.toolName)).toEqual(['getNovelInfo', 'getForeshadowings']);
  });
});
...
describe('toolLabel', () => {
  ...
  it('未知のツール名はそのまま返す（フォールバック）', () => {
    ...
    expect(toolLabel('searchNovelKnowledge-abc123')).toBe('searchNovelKnowledge-abc123');
  });
});
...
describe('ToolActivity コンポーネント', () => {
  ...
  it('input-streaming は「実行準備中...」を表示する', () => {
    const parts = [
      { type: 'tool-getNovelInfo', toolCallId: 'c1', state: 'input-streaming', input: {} },
    ];
    render(<ToolActivity parts={parts} isStreaming={true} />);
