...
function renderList(overrides: Partial<Parameters<typeof ChatSessionList>[0]> = {}) {
  const props = {
    sessions,
...
describe('ChatSessionList', () => {
  ...
  it('onSaveTitle が false を返すと編集モードが開いたままになること', async () => {
    ...
    await waitFor(() => expect(onSaveTitle).toHaveBeenCalledWith('sess-1', 'プロット相談'));
  ...
  it('削除フローで確認後に onDeleteSession が呼ばれること', async () => {
    ...
    expect(screen.getByText('この相談履歴を削除しますか？')).toBeInTheDocument();
