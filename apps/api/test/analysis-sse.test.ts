...
describe("analysis SSE のワイヤ契約", () => {
  ...
  it("check-voice は progress → complete の順で event 名とペイロードを維持すること", async () => {
    ...
    mockServices.analysis.streamCheckVoice.mockImplementation(
      async function* () {
        ...
        yield { result: { ok: true }, savedId: "result-1", type: "complete" };
      }
    );

    const res = await app.request(
      `/api/novels/${NOVEL_ID}/generate/check-voice`,
      {
        body: JSON.stringify({}),
...
describe("vector reindex SSE のワイヤ契約", () => {
  ...
  it("reindex は progress → done の順でイベントを返すこと", async () => {
    mockServices.reindex.reindexAll.mockImplementation(
      async (_configId: unknown, onProgress: (p: unknown) => void) => {
        onProgress({ current: 1, phase: "embedding", total: 3 });
        onProgress({ current: 3, phase: "upserting", total: 3 });
        return { indexed: 3 };
      }
    );
  ...
  it("reindex は例外発生時に event: error / { error } を返すこと", async () => {
    mockServices.reindex.reindexAll.mockRejectedValue(
      new Error("再構築に失敗しました")
    );
