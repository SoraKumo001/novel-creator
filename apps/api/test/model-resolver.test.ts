...
vi.mock('drizzle-orm', async (importOriginal) => {
  ...
  return {
    ...actual,
    eq: vi.fn((column: unknown, value: unknown) => ({ __eqMarker: true, column, value })),
  };
});
...
import { resolveEmbeddingModel, resolveLLMModel } from '../src/core/model-resolver.js';
...
function createMockDb(options: {
  ...
  const from = vi.fn().mockImplementation((table: unknown) => ({
    where: vi.fn().mockImplementation(async (cond: { value?: unknown } | undefined) => {
...
function createMockContext(
  mockDb: unknown,
  overrides: { env?: Record<string, unknown>; llm?: unknown; embedding?: unknown } = {},
): ServiceContext {
  return {
    db: mockDb as never,
...
describe('resolveLLMModel', () => {
  ...
  it('有効な modelConfigId を指定するとその設定でモデルを生成する', async () => {
    ...
    const model = await resolveLLMModel(createMockContext(db), 'cfg-custom', 'throw');

    expect(model).toEqual({ __mock: 'llm-from-config', configId: 'cfg-custom' });
    expect(mockedCreateLLM).toHaveBeenCalledWith(customConfig, expect.anything());
  ...
  it("onMissing: 'throw' で不明な modelConfigId は NotFoundError になりデフォルトへフォールバックしない", async () => {
    ...
    await expect(resolveLLMModel(createMockContext(db), 'cfg-missing', 'throw')).rejects.toThrow(
      new NotFoundError('LLM Config', 'cfg-missing'),
    );
  ...
  it("onMissing のデフォルトは 'throw' として動作する", async () => {
    ...
    await expect(resolveLLMModel(createMockContext(db), 'cfg-missing')).rejects.toThrow(
      NotFoundError,
    );
  });
  ...
  it("onMissing: 'useDefault' で不明な modelConfigId はデフォルト設定を使用する", async () => {
    ...
    const model = await resolveLLMModel(createMockContext(db), 'cfg-missing', 'useDefault');

    expect(model).toEqual({ __mock: 'llm-from-config', configId: 'cfg-default' });
    expect(mockedCreateLLM).toHaveBeenCalledWith(defaultConfig, expect.anything());
  });
  ...
  it('modelConfigId 未指定でデフォルト設定もなければ ctx.llm を使用する', async () => {
    ...
    const model = await resolveLLMModel(createMockContext(db), undefined, 'throw');
...
describe('resolveEmbeddingModel', () => {
  ...
  it('有効な embeddingConfigId を指定するとその設定のモデルと dimensions を返す', async () => {
    ...
    const resolved = await resolveEmbeddingModel(createMockContext(db), 'emb-custom', 'throw');

    expect(resolved.model).toEqual({ __mock: 'embedding-from-config', configId: 'emb-custom' });
  ...
  it("onMissing: 'useDefault' で不明な embeddingConfigId はデフォルト設定を使用する", async () => {
    ...
    expect(resolved.model).toEqual({ __mock: 'embedding-from-config', configId: 'emb-default' });
  ...
  it('embeddingConfigId 未指定はデフォルト設定を使用する', async () => {
    ...
    const resolved = await resolveEmbeddingModel(createMockContext(db), null, 'throw');
  ...
  it('env.EMBEDDING_DIMENSIONS 未設定の場合は 1536 にフォールバックする', async () => {
    ...
    const resolved = await resolveEmbeddingModel(createMockContext(db), undefined, 'useDefault');
