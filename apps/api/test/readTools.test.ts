...
describe("truncateList", () => {
  ...
  it("max を超える配列は切り詰められ、省略明示が生成される", () => {
    ...
    expect(result.notice).toBe(
      "[truncated: showing 30 of 57] 残り 27 件は省略されました"
    );
  });
...
describe("readTools", () => {
  ...
  describe("getSectionContent の novelId スコープ", () => {
    ...
    it("節の親章の novelId がバインドされた novelId と一致する場合、本文を返す", async () => {
      const tools = createReadTools(
        createCtx(
          setupDb({ boundNovelId: NOVEL_ID, chapterNovelId: NOVEL_ID })
        ),
        NOVEL_ID
      );
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    it("節の親章の novelId が不一致の場合、エラーオブジェクトを返す", async () => {
      const tools = createReadTools(
        createCtx(
          setupDb({ boundNovelId: NOVEL_ID, chapterNovelId: OTHER_NOVEL_ID })
        ),
        NOVEL_ID
      );
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      const tools = createReadTools(
        createCtx(
          setupDb({
            ...
            chapterNovelId: NOVEL_ID,
            contentRow: [
              { body: longBody, id: "content-1", sectionId: SECTION_ID },
            ],
          })
        ),
        NOVEL_ID
