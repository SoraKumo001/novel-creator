...
システムの詳細な設計・仕様・機能解説は [`doc/`](./doc/README.md) 配下の各ドキュメントを参照してください。

| ドキュメント                                                          | 主な内容                                                                   |
| :-------------------------------------------------------------------- | :------------------------------------------------------------------------- |
| 📖 **[システムアーキテクチャ](./doc/architecture.md)**                | モノレポ構成、Hono RPC、SSEストリーミング、Cloudflareハイブリッド設計      |
...
- **執筆・エディタ・プレビュー環境**:
  ...
  - **🔀 複数候補のバリエーション並列生成 & 比較**: 1〜3案の同時ストリーミング生成、タブ切替および並列比較（Split View）表示、案ごとの文字数確認と選択置換・挿入
...
- **設定・登場人物の管理 & 整合性支援**:
  ...
  - **🎭 キャラクター口調・一貫性チェッカー (`CharacterVoiceCheckerModal`)**: 人物設定（一人称・二人称・語尾・性格）と本文を照合し、キャラ崩壊・口調ブレを検出して改善案を提示・反映
  - **📊 登場人物出現頻度ヒートマップ (`CharacterHeatmapModal`)**: 全章節×人物のマトリックス表示、出番なしキャラ（⚠️）の自動検出
  - **⚡ 設定変更の影響範囲シミュレーター (`ImpactAnalysisModal`)**: 設定変更前後の差分から、影響を受ける全プロット・章節・年表・伏線への影響を予測
  - Mermaid.js による人物相関図の自動生成・可視化
  - VectorDB への Embedding 保存・セマンティック検索
  - **🧬 埋め込み（Embedding）モデル設定 & インデックス全再構築 (`/settings`)**: UI上からの埋め込みモデル・次元数追加・変更および、ワンクリックでの全小説データベクトル一括再構築（SSEプログレス表示付き）

- **ストーリー分析 & AIペルソナレビュー**:
  - **📈 感情アーク & テンション可視化 (`StoryArcChartModal`)**: 全章節の「緊張感（Tension）」と「感情価（Valence）」をAIスコアリングし、SVG折れ線グラフで起伏を描画・中だるみを指摘
  - **👥 4ペルソナによる模擬読者・編集部レビュー (`MultiPersonaReviewModal`)**: 商業編集者、一般読者、設定考察派、辛口評論家の4視点から星評価・キャッチコピー・講評・リライト助言を生成
  - **進捗表示つき分析実行**: 各分析は SSE ストリーミングで進捗（本文収集 → AI分析 → 保存）をリアルタイム表示、キャンセル可能
  - **分析結果の自動保存 & 履歴**: すべての分析結果は DB に自動保存され、モーダル内「履歴」からいつでも再確認・再実行・削除可能
  - **日本語出力の強制**: 分析プロンプトが出力言語を日本語に固定（非日本語レスポンスを防止）
- **AI 創作相談チャット（Creative Chat & Agentic Tool Calling）**:
  ...
  - **自律的設定提案 (Propose Tools) & インライン承認カード (`ChatProposalCard`)**: 会話中に考案された人物・世界観設定・伏線・年表イベント・章プロットを AI が自律提案し、チャット上でワンクリック直接反映
  - ツール実行状態を視覚的に表示する **ToolActivity** コンポーネント
  - **全方位エンティティ抽出モーダル (`ChatInsertEntityModal`)**: チャットログから人物・設定・伏線・年表・プロットをAI一括抽出し、上書き/追記マージ/新規追加を選択して小説データへ反映
- **伏線 & 時系列 & 履歴管理**:
