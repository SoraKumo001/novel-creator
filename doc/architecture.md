...
| :-------------------- | :-------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`apps/web`**        | React 19, Vite, TanStack Router/Query, Tailwind CSS v4, Monaco Editor | ユーザーインターフェース。ファイルベースルーティングによる画面遷移、MarkdownエディタとカードUIのシームレスな切り替え、`SaveStatusBadge` による保存状態可視化、SSEストリーミング受信処理。 |
| **`apps/api`**        | Hono, `@hono/node-server`, Cloudflare Workers (`wrangler`)            | API サーバー。Hono RPC による型安全なエンドポイント提供、SSE ストリーミング生成配信、DB / VectorStore / LLM サービスの結合、MCP サーバーホスト。                |
...
| **`packages/db`**     | Drizzle ORM, `postgres`, `pgvector`                                   | DB スキーマ定義、マイグレーション管理（`db:migrate`）、スキーマリセット（`db:reset`）、リレーショナルデータアクセス。                                              |
