...
- **自律的設定提案 (Propose Tools) & インライン承認カード (`ChatProposalCard`)**:
  会話の中で新しいアイデアが固まった際、LLM が 5 種類の提案ツール（`proposeCreateCharacter`, `proposeCreateSetting`, `proposeAddForeshadowing`, `proposeAddTimelineEvent`, `proposeUpdatePlot`）を自律的に呼び出します。チャット画面上にリッチな提案カードが表示され、作家はワンクリックで即座に小説データ（人物・世界観・伏線・年表・プロット）へ反映できます。
- **ツール実行状態の可視化 (`ToolActivity`)**:
  参照ツールや提案ツールの実行中・完了状態や引数・戻り値をチャット画面内に分かりやすくカード表示。
- **全方位エンティティ抽出・反映モーダル (`ChatInsertEntityModal`)**:
  チャットログ全体から、決定された「登場人物」「世界観設定」「伏線」「時系列・年表」「プロット（章構成）」を AI が自動抽出（`extractChatEntities`）。作家は 5 つのタブから各項目を確認し、**「新規追加」「上書き更新」「追記マージ」** を選択して小説データへ一括反映できます。
