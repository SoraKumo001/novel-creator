...
```mermaid
flowchart TD
    Start([創作開始]) --> Step0[0. アイデア発想 & 着想メモ保存 ideas]
    Step0 -->|ブレスト・採用| Step1[1. 構想深掘り & 創作チャット]
...
### 2.0 アイデア帳 & ストーリーシード発想 (`ideas`)

- **着想メモの蓄積とステータス管理**:
  創作初期の断片的なアイデアやプロットの種を、タイトル・本文・出典（source）とともに `ideas` テーブルにストック。ステータスは `draft`（下書き・検討中）、`adopted`（採用済み）、`rejected`（不採用）で管理されます。
- **MCP & AI ガイドフロー連携**:
  MCP 経由のブレストプロンプト（`brainstorm_plot`, `twist_ideas`, `what_if_brainstorm`, `draw_story_seeds`）で発散させた案を `create_idea` で即時保存し、`evaluate_ideas` で採点評価した後に `set_idea_status` で採用、最終的に `idea_to_outline` を通じて章・節・伏線へシームレスに展開できます。
...
- **Agentic な小説データ参照 (Read Tools)**:
  LLM が自律的に 8 種類の専用参照ツールを実行（小説情報、人物、設定、プロット、節本文、伏線、年表、ベクトル検索）。ユーザーが細かい文脈を前提にせず質問しても、必要な背景データを自分で取得して的確に回答します。
- **自律的設定提案 (Propose Tools) & 登録用提案カード (`ChatProposalCard`)**:
  会話の中で新しい設定や修正案が固まった際、LLM が 7 種類の提案ツール（`proposeCreateCharacter`, `proposeCreateSetting`, `proposeAddForeshadowing`, `proposeAddTimelineEvent`, `proposeUpdatePlot`, `proposeUpdateStoryOutline`, `proposeBulkCreate`）を自律的に呼び出します。
  - **最適な視線誘導レイアウト**: 思考プロセスや検索ログは上部にコンパクトにまとめ、**作成された登録用提案カードは AI の解説テキスト直下（メッセージ最下部）に配置**。長文出力時でも画面を上へスクロールして戻す必要がなく、読み終えた視線の位置ですぐに確認・反映が可能です。
  - **置換・削除対応**: 設定や人物の改名・刷新提案時には、旧設定の自動削除と新設定の登録がワンアクションで安全に処理されます。
- **Monaco DiffEditor による精密な差分確認 (`ProposalDiffModal`)**:
  提案カードの「🔍 差分を確認」から、変更前後の Markdown 全文差分を Monaco DiffEditor 上で確認可能。
  - **左右並列（2ペイン）／行内統合（1ペイン）の切り替え**: 画面幅や好みに応じて表示モードを即座に変更可能。
  - **一括登録（Bulk）時の複数カテゴリタブ切り替え**: 一括提案に含まれる全カテゴリ（登場人物、世界観・設定、伏線、年表）を上部ピル型タブで自由に切り替えて比較可能。どのタブを表示していても、ワンクリックで全カテゴリを一括保存できます。
  - **Markdown エディタ直接遷移**: 各カテゴリの Markdown 編集画面へジャンプしてさらに微調整することも可能です。
- **ツール実行状態の可視化 (`ToolActivity`)**:
  LLM が小説のプロット、設定、人物、本文、伏線、年表、ベクトル検索を自律的に実行している様子をリアルタイムにバッジやアコーディオンで表示。実行中のツール、引数、実行結果の詳細をいつでも展開して確認できます。
...
4. **章・節の並び替え・移動 & 節メタ直接編集**:
   - ドラッグ＆ドロップおよび移動アクションにより、物語の構成順を柔軟に変更可能。
   - 執筆画面（`/novels/:id/chapters/:chId/sections/:secId`）から章・節のタイトルや概要（あらすじ）を直接モーダルで編集可能。
5. **章情報を考慮した本文ストリーミング執筆**:
   - 当該章のタイトル・概要、前節の末尾、現在の節概要、関連設定・人物、作品の文体ガイドラインを統合してコンテキストに注入。
...
- **リアルタイム保存状態インジケーター (`SaveStatusBadge`)**:
  - エディタツールバーに「保存完了」「保存中...」「未保存の変更あり」「保存エラー」のステータスバッジを常時表示。
  - バックグラウンド自動保存と手動保存の双方が視覚的にフィードバックされ、執筆データの喪失を防止。
...
- **インライン AI 推敲・加筆 (`InlineAIAssistant`)**:
  ...
  - 標準アクション（「描写を深める」「心理・感情強化」「会話をテンポよく」「簡潔にする」「別の言い回し」）、自由指示、および**ユーザー登録のカスタムプロンプト**をワンクリックで実行。
- **🔀 複数候補のバリエーション並列生成 & 比較 (Multi-Variant Generation)**:
  - 1回の指示で「1案」「2案」「3案」の候補数を指定可能。
  - 複数の候補が並列かつリアルタイムにストリーミング生成されます。
  - **タブ切替**（案1 / 案2 / 案3 の文字数付きタブ）および **2カラム並列比較（Split View）** で候補を俯瞰・比較。
  - 選択したお気に入りの案を「✍️ 選択範囲を置換」または「直後に挿入」で即座に本文へ反映。
- **🪄 カスタムプロンプト作成・管理機能 (`CustomPromptModal` / `CustomPromptManagerModal`)**:
  - 設定ページ（`/settings`）またはエディタ上から、独自の推敲・生成・相談プロンプトを無制限に作成・登録可能。
  - `{selectedText}`, `{surroundingText}`, `{characters}`, `{settings}`, `{styleGuide}`, `{instruction}` などの変数タグをワンクリックで挿入できるテンプレートビルダー。
  - 「作品専用プロンプト」または「全作品共通プロンプト」のスコープ設定。
  - 初期導入時に使える標準プリセット（ハードボイルド調変換、五感と環境音の肉付け、サスペンス緊張感アップなど）の自動シードとワンクリック復元。
- **🎯 執筆目標 & 進捗トラッキング**:
